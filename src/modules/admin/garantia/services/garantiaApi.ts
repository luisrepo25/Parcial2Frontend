...
export const garantiaApi = {
  ...
  /**
   * Obtener garantías por marca
   * GET /garantias/marca/{marcaId}
   */
  getGarantiasByMarca: async (marcaId: number): Promise<Garantia[]> => {
    try {
      const response = await fetch(`${API_URL}garantias/marca/${marcaId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          ...authApi.getAuthHeaders(),
      ...
      const data = await response.json();
      console.log("Respuesta de garantías por marca:", data);
      ...
      console.error(`Error al obtener garantías por marca ${marcaId}:`, error);
      throw error;
    }
  },
};
